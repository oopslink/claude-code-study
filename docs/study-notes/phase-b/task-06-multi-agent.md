...
克隆的优势：
  1. 每个 agent 有自己的状态副本，修改互不影响（内存隔离）
...
2. **共享内存状态**：通过 `AsyncLocalStorage` 隔离上下文同时共享 `AppState` Store
...
| `local_agent` | 本地进程内（同线程） | 内存隔离（克隆上下文） | coordinator 模式的 worker；需要 LLM 完成任务 | 需要 LLM、可后台并行、结果异步通知 |
...
3. **隔离层次与执行方式正交**：LocalAgentTask 和 InProcessTeammateTask 都在本地进程内运行，但隔离机制不同——前者通过克隆 ToolUseContext 实现内存隔离，后者通过 AsyncLocalStorage 实现上下文隔离。
